...
async function run(projectRoot, options = {}) {
  ...
  let displayRows = filterRowsForTable(sorted, preferences);
  let expandedTableBecauseNoVulnMatches = false;
  if (
    ...
    displayRows = sorted;
    expandedTableBecauseNoVulnMatches = true;
  }

  if (printConsole) {
    printConsoleTable(displayRows);
    if (expandedTableBecauseNoVulnMatches) {
      console.log(
        chalk.dim(
          'No packages matched your vulnerability filter (nothing to list as “vulnerable only”). Showing all analyzed packages in the table above.',
        ),
      );
    } else if (displayRows.length < sorted.length) {
      console.log(
        chalk.dim(
